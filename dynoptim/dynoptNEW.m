function [optimout,optim_param] = dynoptNEW(optim_param)
...
if sum(strcmp(fieldnames(optim_param.options), 'GradObj')) == 1
    objgr = optim_param.options.GradObj;
else
    optim_param.options.GradObj = 'off';
    objgr = optim_param.options.GradObj;
end

if sum(strcmp(fieldnames(optim_param.options), 'GradConstr')) == 1
    congr = optim_param.options.GradConstr;
else
    optim_param.options.GradConstr = 'off';
    congr = optim_param.options.GradConstr;
end

if sum(strcmp(fieldnames(optim_param.options), 'NLPsolver')) == 1
    solver = optim_param.options.NLPsolver;
else
...
if (strcmp(objgr,'on') == 1) && (strcmp(congr,'on') == 1)
    ...
    if (strcmp(solver,'ipopt') == 1)
        ...
        warning('Gradients are missing, you cannot use Ipopt, program continues with fmincon.')
        optim_param.options.NLPsolver = 'fmincon';
