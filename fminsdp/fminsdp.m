...
%                                       beginning of the first matrix
%                                       constraint.
%                                       Default: 1
%
% sp_pattern     (sparse) matrix or  -- Sparsity pattern of Cholesky factors.
%                cell array of          These can be generated by a call to
%                sparse matrices        symbol_fact. If this option is used,
%                                       the user must provide one sparsity
%                                       pattern for each matrix constraint.
%                                       Default: []
%
%
...
%                                       NOTE: If you run IPOPT older than
%                                             3.11.0, make sure to modify the file 'ipopt_main'
%                                             appropriately.
%                                       Default: 'fmincon'
%
...
%
%
% See also FMINCON, SDPOPTIONSET, SYMBOL_FACT, SVEC, NONLCONSDP, HESSIANSDP
...
elseif isa(options.NLPsolver,'char')
    switch lower(options.NLPsolver)
        case {'fmincon','snopt','ipopt','knitro','mma','gcmma'}
        otherwise
            error('Unknown value for options.NLPsolver. Available options are ''fmincon'', ''snopt'', ''knitro'', ''ipopt'', ''mma'' or ''gcmma''.');
    end
else
...
%
% Else continue with matrix inequality constraints
%
...
elseif isa(options.c,'numeric') && isscalar(options.c)
    ...
    if s_low>s_upp
        error('Inconsistent bounds on auxiliary variables s. Check options.s_low and options.s_upp.');
    end
else
...
for i = 1:nMatrixConstraints
    ...
    if (isnumeric(options.sp_pattern{i}) || islogical(options.sp_pattern{i}))  && ...
            ~isempty(options.sp_pattern{i})
        
        % Get symbolic Cholesky factorzation of constraint matrices
        sp_L{i} = tril(logical(options.sp_pattern{i}));
        L_size(i) = size(sp_L{i},1);
        ...
        % the ceq vector
        ind(i+1)  = ind(i)  + nnz(sp_L{i});
        
        % Find out which l_{p} maps to which L_{ij}
        Lindz{i} = zeros(nnz(sp_L{i}),2);
        [Lindz{i}(:,1),Lindz{i}(:,2)] = find(sp_L{i});
        
        ceqind(Lind(i):Lind(i+1)-1,1) = offset+sub2ind_tril(L_size(i),Lindz{i}(:,1),Lindz{i}(:,2));
        offset = offset + L_size(i)*(L_size(i)+1)/2;
...
for i = 1:nMatrixConstraints
    ...
    if isempty(options.L0)
        ...
        if ~isfield(options.eigs_opts,'v0') || size(options.eigs_opts.v0,1)~=size(A,1)
            options.eigs_opts.v0 = ones(size(A,1),1);
        end
        
        % Compute minimum eigenvalue of A. If negative,
        % add the identity matrix times the magnitude of
        % this eigenvalue.
        d = eigs(A,1,'SA',options.eigs_opts);
        
        if options.c>0
            s0 = max(s0,-min(d,0) + 1e-12);
            % Cholesky factor are computed in a separate loop below
        else
            if d<1e-13
        ...
        % User has provided initial guesses for each of the matrix
        % constraints
        if options.c>0
            A = smat(vA(Lind(i):(Lind(i+1)-1),1),sp_L{i});
            if ~isfield(options.eigs_opts,'v0') || size(options.eigs_opts.v0,1)~=size(A,1)
...
% Compute initial guesses for the Cholesky factors when running feasibility
% mode and there are no user supplied guesses.
if options.c>0 && isempty(options.L0)
    s0 = max(s0,1e-12);
    for i = 1:nMatrixConstraints
        A = smat(vA(Lind(i):(Lind(i+1)-1),1),sp_L{i});
        % NOTE: When running Matlab R2012 with Blas and Lapack from Atlas 3.10.1
        %       on Ubuntu 12.10 64-bit it appears that chol does not
        %       work for dense matrices, hence the "sparse" below.
        % Attempt Cholesky, if it fails increase s0 by a factor
        while true
            try
...
elseif strcmpi(options.GradConstr,'on')
    ...
    % svec(A) = Sn*vec(A)
    %
    % See hessianSDP.m for its usage
    n = size(sp_Lblk,1);
    col = find(sp_Lblk==1);
    row = 1:numel(col);
...
if isfield(options,'Hessian') && ~isempty(options.Hessian)
    ...
    if isa(options.Hessian,'char') && strcmpi(options.Hessian,'user-supplied') && ~strcmpi(options.NLPsolver,'snopt')
        ...
        if strcmpi(options.HessianCheck,'on') && ~isempty(options.HessFcn)
            ...
            if ~isempty(options.HessFcn)
                ...
                lambda.eqnonlin   = zeros(nceq,1);
                lambda.ineqnonlin = zeros(ncineq,1);
            ...
            if ncineq ~= 0 && ~isempty(options.HessFcn)
                ...
                lambda.eqnonlin   = zeros(nceq,1);
                lambda.ineqnonlin = ones(ncineq,1);
                
                H_findiff = hessian(@(x) Lagrangian(x,lambda,@nonlconSDP,data),x0(1:nxvars,1));
                H_analytical = options.HessFcn(x0(1:nxvars,1),lambda);
        ...
        if strcmpi(options.HessianCheck,'on') && ~(isempty(options.HessFcn) && strcmpi(options.HessianCheckLL,'off'))
            
            % Check Hessian of equality constraints wrt to primary and
            % auxiliary variables (the latter is optional).
            
            lambda.eqnonlin   = ones(nceq,1);
            lambda.ineqnonlin = zeros(ncineq,1);
            ...
            elseif strcmpi(options.HessianCheckLL,'on')
                ...
                try
                    H_findiff = hessian(@(x) Lagrangian(x,lambda,@nonlconSDP,data),x0(:));
                catch msg
                    error(['Unable to compute finite difference estimation for Hessian. ' ...
                        'Make sure DERIVESTsuite is on your Matlab path and that the ' ...
                        'function ''hessian'' is not shadowed on the path by some other ' ...
                ...
                H_analytical = hessianSDP(x0,lambda,options.HessFcn,data);
        ...
        if isempty(options.HessMult)
            ...
            options.HessFcn = @(x,lambda) hessianSDP(x,lambda,UserHessFcn,data);
...
% Call selected NLP-solver to solve the reformulated problem
switch lower(options.NLPsolver)
    
    case 'fmincon'
        
        [x,fval,exitflag,output,lambda,grad,Hessian] = ...
            fmincon(objfun_,x0,a,b,aeq,beq,lb,ub,@(x) nonlconSDP(x,data),options);
        
    case 'snopt'
        
        [x,fval,exitflag,output,lambda,grad,Hessian] = ...
            snopt_main(objfun_,x0,a,b,aeq,beq,lb,ub,@(x) nonlconSDP(x,data),options,...
            data,ceq,cineq);
        
    case 'ipopt'
        
        [x,fval,exitflag,output,lambda,grad,Hessian] = ...
            ipopt_main(objfun_,x0,a,b,aeq,beq,lb,ub,@(x) nonlconSDP(x,data),options,...
            data,ceq,cineq);
        
    case 'knitro'
        
        [x,fval,exitflag,output,lambda,grad,Hessian] = ...
            knitro_main(objfun_,x0,a,b,aeq,beq,lb,ub,@(x) nonlconSDP(x,data),options,...
            data,ceq,cineq);
        
    case {'mma','gcmma'}
        
        [x,fval,exitflag,output,lambda,grad,Hessian] = ...
            mma_main(objfun_,x0,a,b,aeq,beq,lb,ub,@(x) nonlconSDP(x,data),options,...
            data,ceq,cineq);
...
if nargout>0
    ...
    if nargout>3
        ...
        [unused,ceq] = data.nonlcon(x(1:nxvars));
        ...
        for i = 1:nMatrixConstraints
            % Constraint matrices at solution
            output.A{i} = smat(vA((Lind(i):(Lind(i+1)-1))-nxvars,1),sp_L{i});
            % Cholesky factors at solution
